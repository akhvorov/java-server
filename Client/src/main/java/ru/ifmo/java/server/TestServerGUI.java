...
//            ClientsRunner clientsRunner = new ClientsRunner(queriesNum, clientsNum, listSize, delay);
//            clientsRunner.runTest(n, m, x, delay, changeName, from, to, step, serverIp, serverType);
//
//            List<Integer> valRange = clientsRunner.getValRange();
//            Map<ClientsRunner.TimeType, List<Double>> times = clientsRunner.getTimes();
//
//            for (ClientsRunner.TimeType timeType : ClientsRunner.TimeType.values()) {
//                List<Double> time = times.get(timeType);
//                List<XYChart.Data<Integer, Double>> dataset = new ArrayList<>();
//                for (int i = 0; i < valRange.size(); i++) {
//                    dataset.add(new XYChart.Data<>(valRange.get(i), time.get(i)));
//                }
//                data.get(timeType.ordinal()).add(new XYChart.Series(serverType, FXCollections.observableArrayList(dataset)));
//            }
//        }
//
//        Platform.runLater(() -> {
